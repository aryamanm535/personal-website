...
export default function StarField() {
  ...
  useEffect(() => {
    ...
    const draw = (ts) => {
      ...
      clickStarsRef.current.forEach(s => {
        ...
        // Scroll with same SCROLL_FACTOR as background stars
  ...
  useEffect(() => {
    const onClick = (e) => {
      ...
      // Store scrollY at click so the star can scroll with parallax each frame
      clickStarsRef.current.push({ x: vx, y: vy, scrollY, born });

      // Find nearby background stars and keep a live reference to each
      const nearby = [];
      stars.forEach(s => {
