...
const CONNECT_R    = 140;
const MAX_CONNECT  = 5;
const SCROLL_FACTOR = 0.5;  // how much stars scroll relative to page
...
export default function StarField() {
  ...
  useEffect(() => {
    ...
    const draw = (ts) => {
      ...
      // Scroll offset: stars shift UP as user scrolls down (parallax behind content)
      const scrollOff = (window.scrollY * SCROLL_FACTOR) % H;
      ...
      starsRef.current.forEach(s => {
        ...
        // Subtract scroll offset so stars move UP as you scroll down
        ...
        ctx.fillStyle = `rgba(${s.bright ? (isLight ? '90,30,160' : '230,200,255') : (isLight ? '110,50,180' : '200,170,240')},${Math.max(0.08, alpha)})`;
        ctx.beginPath();
      ...
      linesRef.current.forEach(line => {
        ...
        const fadeOut = age > LINE_FADE_MS
          ? Math.max(0, 1 - (age - LINE_FADE_MS) / 500) : 1;
        const alpha   = 0.6 * drawIn * fadeOut;
        if (alpha < 0.01) return;

        const ex = line.x1 + (line.x2 - line.x1) * drawIn;
        const ey = line.y1 + (line.y2 - line.y1) * drawIn;
        ...
        [{ x: line.x1, y: line.y1 }, { x: ex, y: ey }].forEach(({ x, y }) => {
  ...
  useEffect(() => {
    const onClick = (e) => {
      ...
      clickStarsRef.current.push({ x: vx, y: vy, born });

      // Search using each star's current drawn Y
      const nearby = [];
      stars.forEach(s => {
      ...
      nearby.slice(0, MAX_CONNECT).forEach(({ s, drawnY }) => {
        linesRef.current.push({ x1: vx, y1: vy, x2: s.x, y2: drawnY, born });
      });
    };
